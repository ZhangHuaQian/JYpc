<template>
  <div class="home-container">
    <el-row class="news-list" justify="space-between">
      <router-link :to="{path:'/common/details1', query: {itemid: newsList.length>0?newsList[0].id:'', siteId: newsList.length>0?newsList[0].siteId:''}}">
        ...
        <el-row class="others-news-item">
          <template v-for="item in columnList">
            <template v-if="item.name === '新闻'">
              <router-link :key="item.id" :to="{path:'/common/commonlist1', query: {columnId: 67, columnName: '招生新闻'}}" tag="p" style="text-align: right;color: #666666;cursor:pointer;">更多新闻></router-link>
            </template>
          </template>
        </el-row>
      </el-col>
    ...
    <el-row class="zsxx">
      <el-row>
        ...
        <el-col :md="2">
          <div style="text-align: left;line-height: 20px;font-size:20px;font-weight:bold;">
            招生信息
          </div>
        </el-col>
        <el-col :md="14">
          <div style="text-align: right;margin-right: 25px;line-height: 20px;color:rgba(102,102,102,1);">
            <template v-for="item in columnList">
              <template v-if="item.name === '招生信息'">
                <router-link :key="item.id" :to="{path:'/common/commonlist1', query: {columnId: item.id, columnName: item.name}}">更多></router-link>
              </template>
            </template>
          </div>
        </el-col>
        ...
        <el-col :md="6">
          <a style="text-align: left;line-height: 22px;font-size:20px;font-weight:bold;">
            更多入口
          </a>
        </el-col>
      </el-row>
      <el-row>
        ...
        <el-col :span="8">
          <template v-for="item in gengduorukou">
            <router-link :key="item.id" :to="{path:'/common/details1', query: {itemid: item.id, siteId: item.siteId}}">
              <el-row style="padding-right: 0;"><img :src="item.image"></el-row>
            </router-link>
          </template>
        </el-col>
      </el-row>
    ...
    <el-row class="jy">
      <el-col class="jy-left" :xs="6" :md="12">
        <el-row style="height: 40px; border-bottom: 2px solid rgba(210,210,210,1);line-height: 40px;">
          <div style="height: 40px; border-bottom: 2px solid rgba(210,210,210,1);line-height: 40px;position: relative;">
            <div style="display: inline-block;width: 119px;height: 40px;border-bottom: 2px solid rgba(19,107,185,1);line-height: 40px;">
              <span style="position: absolute;top: 3px;">
                <i style="display: inline-block;height: 22px;" class="jy-left-icon1" />
              </span>
              <span class="jys" style="height: 40px;line-height: 40px;margin-left: 30px;">
                就业政策
              </span>
            </div>
          ...
          <div class="list">
            <el-row v-for="item in jiuyezhengce" :key="item.id" class="list-item">
              <router-link :to="{path:'/common/details1', query: {itemid: item.id, siteId: item.siteId}}">
                <el-col style="width: 18px;height: 18px;margin-right: 4px;" :span="1"><i class="list-item-point"><img src="../../assets/secondaryHome/t.png"></i></el-col>
                <el-col :span="18"><p>{{ item.title }}</p></el-col>
                <el-col :span="4" :offset="1"><p>{{ getMoment(item.modifyTime, 'YYYY-MM-DD') }}</p></el-col>
              </router-link>
            </el-row>
          </div>
        </el-row>
      ...
      <el-col class="jy-right" :xs="6" :md="12">
        <el-row>
          <div style="height: 40px; border-bottom: 2px solid rgba(210,210,210,1);line-height: 40px;position: relative;">
            <div style="display: inline-block;width: 119px;height: 40px;border-bottom: 2px solid rgba(19,107,185,1);line-height: 40px;">
              <span style="position: absolute;top: 3px;"><i style="display: inline-block;height: 22px;" class="jy-right-icon2" /></span>
          ...
          <div class="list">
            <el-row v-for="item in jiuyefudao" :key="item.id" class="list-item">
              <router-link :to="{path:'/common/details1', query: {itemid: item.id, siteId: item.siteId}}">
                <el-col style="width: 18px;height: 18px;margin-right: 4px;" :span="1"><i class="list-item-point"><img src="../../assets/secondaryHome/t.png"></i></el-col>
                <el-col :span="18"><p>{{ item.title }}</p></el-col>
                <el-col :span="4" :offset="1" style="text-align: right;"><p>{{ getMoment(item.modifyTime, 'YYYY-MM-DD') }}</p></el-col>
              </router-link>
            </el-row>
          </div>
        </el-row>
