        ...
        <el-col :span="20">
          ...
          >
            <template v-for="item in menu">
              <!-- <el-menu-item
                v-if="item.name==='部门概况'&&item.name!=='工作职责'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`/common/commonlist2/list2?siteId=${$route.query.siteId}&columnId=${item.id}&columnName=${item.name}`"
              >{{ item.name }}</el-menu-item>
              <el-menu-item
                v-if="item.name==='规章制度'&&item.name!=='工作职责'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`/common/commonlist2/list2?siteId=${$route.query.siteId}&columnId=${guizhangzhidu[0].columnId}&columnName=${item.name}`"
              >{{ item.name }}</el-menu-item>
              

              <el-menu-item
                v-if="item.name==='学院概况'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`${item.url}`"
              >{{ item.name }}</el-menu-item>
              <el-menu-item
                v-if="item.name==='课程建设'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`${item.url}`"
              >{{ item.name }}</el-menu-item>
              <el-menu-item
                v-if="item.name==='师资队伍'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`${item.url}`"
              >{{ item.name }}</el-menu-item>
              <el-menu-item
                v-if="item.name==='教学科研'&&item.name!=='学校概况'&&item.name!=='学术研究'&&item.name!=='队伍建设'&&item.name!=='公共服务'&&item.name!=='校园文化'"
                :key="item.id"
                :index="`${item.url}`"
              >{{ item.name }}</el-menu-item>-->
              <el-submenu
                ...
                :key="item.id"
                :index="`${item.url}?columnId=${item.id}&columnName=${item.name}`"
              >
                <template slot="title">{{ item.name }}</template>

                <el-menu-item
                  v-if="item.child[0].id==54"
                  :key="item.child[0].id"
                  :index="`/departmentHome?siteId=45`"
                >图书馆</el-menu-item>

                <el-menu-item
                  v-if="item.child[1].id==55"
                  :key="item.child[1].id"
                  :index="`/departmentHome?siteId=49`"
                >档案馆</el-menu-item> 
                ...
                <el-menu-item
                  ...
                  :key="childItem.id"
                  :index="`${item.url}?columnId=${item.id}&columnName=${item.name}&childId=${childItem.id}`"
